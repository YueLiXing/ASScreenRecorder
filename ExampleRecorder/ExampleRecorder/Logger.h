...
// LxLog
#define LxLog(format, ...) \
PrivateLog(0, "Lx", StarBackTrace, format, ##__VA_ARGS__)

// NSLog
#define NSLog(format, ...) \
PrivateLog(0, NSLogTitle, StarBackTrace, format, ##__VA_ARGS__)
//printf("%s [Log] %s\n", star_current_time(), [NSString stringWithFormat:(format),##__VA_ARGS__].UTF8String)
// WSLog
#define WSLog(format, ...) \
PrivateLog(0, "YHL", StarBackTrace, format, ##__VA_ARGS__)
...
#define LxLog(...) while(0){}
#define WSLog(...) while(0){}
